...
```
.
├── main.go                          # Go application (ping-pong HTTP server)
├── go.mod                           # Go module (pinned to 1.25.9)
├── Dockerfile                       # Multi-stage build: golang:alpine → scratch
├── Makefile                         # Developer workflow (up/down/restart/test)
│
├── .github/workflows/
│   ├── pr.yaml                      # PR validation (test, lint, scan, integration)
│   ├── release-please.yaml          # Automated versioning from conventional commits
│   └── release.yaml                 # Multi-arch build, publish, and binary release
│
├── charts/common/                   # Reusable Helm chart (any microservice can use)
│   ├── Chart.yaml
│   ├── values.yaml                  # Defaults: security context, probes, rolling update
│   └── templates/
│       ├── deployment.yaml
│       ├── service.yaml
│       ├── httproute.yaml           # Gateway API routing
│       ├── secret.yaml              # For local/CI (--set at deploy time)
│       ├── externalsecret.yaml      # For staging/production (ESO from cloud store)
│       └── serviceaccount.yaml
│
├── deploy/ping-pong/                # Service-specific deployment config
│   ├── Chart.yaml                   # Depends on charts/common
│   └── values.yaml                  # Image tag, replicas, env, affinity, routes
│
├── docs/local/
│   └── gateway.yaml                 # Kind cluster Gateway API infrastructure
│
├── ARCHITECTURE.md                  # This file — full technical documentation
├── DEPLOYMENT.md                    # Developer quick-start guide
├── EVALUATION.md                    # Answers to evaluation questions
